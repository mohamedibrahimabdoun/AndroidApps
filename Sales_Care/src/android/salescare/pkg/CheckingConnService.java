			...
			public void onReceive(Context context, Intent intent) {
				...
				try {
					...
					if (con_mgr.getNetworkInfo(0).getState() == NetworkInfo.State.CONNECTED) {
						...
						/*
						 * imgAdapter = new ImageDbAdapter(context);
						 * imgAdapter.open();
						 */
						...
						if (cnt > 0) {
							...
							do {
								...
								/*_id = c.getString(c.getColumnIndex("COL_ID"));
								NAME = c.getString(c.getColumnIndex("COL_NAME"));
								MSISDN = c
										.getString(c.getColumnIndex("COL_MSISDN"));
								SERAIL = c
										.getString(c.getColumnIndex("COL_SERAIL"));
								IMSI = c.getString(c.getColumnIndex("COL_IMSI"));
								ADDRESS = c.getString(c
										.getColumnIndex("COL_ADDRESS"));
								IDTYPE = c.getString(c
										.getColumnIndex("COL_IDTYPES"));
								DATE = c.getString(c.getColumnIndex("COL_DATE"));
								FrontImgName = c.getString(c
										.getColumnIndex("COL_FRONTIMGNAME"));
								BackImgName = c.getString(c
										.getColumnIndex("COL_BACKIMGNAME")); 
								FrontImageByte = c.getBlob(c
										.getColumnIndex("COL_FRONTIMAGE"));
								BackImageByte = c.getBlob(c
										.getColumnIndex("COL_BACKIMAGE"));*/
								 ...
								 * obj.setPOSTYPE(POSTYPE); obj.setOWNER(OWNER);
								 * obj.setADDRESS(ADDRESS);
								 * obj.setMOBILE(MOBILE); obj.setBRAND(BRAND);
								 * obj.setSTOCK(STOCK); obj.setDATE(DATE);
								 * obj.setIMEI(IMEI);
